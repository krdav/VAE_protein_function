...
def main (input_file, max_reads,theta):

    data=pdataframe_from_alignment_file(input_file,max_reads)
    indices=index_of_non_lower_case_dot(data.iloc[0]["sequence"])
    data["seq"]=list(map(prune_seq,data["sequence"]))

    new_weights=reweight_sequences(data["seq"],theta)

    filename=input_file[:input_file.index("plmc")]+"t"+str(theta)+"_r"+str(max_reads)+".reweight"

    with open(filename,"wb") as outfile:

        np.save(outfile,np.array(new_weights))
...
if __name__=="__main__":
    ...
    main (input_file,max_reads,theta)
